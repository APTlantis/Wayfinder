...
from .scanner import diagnostics_for_entity, is_complete_entity, match_entities, scan_workspace
...
def _parser() -> argparse.ArgumentParser:
    ...
    discover.add_argument("--diagnostics", action="store_true", help="Include workspace-wide discovery diagnostics.")
...
def _text_discover(entities: list[Entity]) -> str:
    lines = ["ID\tTITLE\tKIND\tLIFECYCLE\tPHYSICAL PATH"]
    lines.extend(f"{item.id or '-'}\t{item.title or '-'}\t{item.kind or '-'}\t{item.lifecycle or '-'}\t{item.physical_path}" for item in entities)
    return "\n".join(lines)
...
def _run(argv: list[str] | None = None) -> int:
    ...
    if args.command == "discover":
        entities = scan.entities if args.include_incomplete else [entity for entity in scan.entities if is_complete_entity(entity)]
        diagnostics = scan.diagnostics if args.diagnostics else []
        data = {
            "workspace_root": str(scan.workspace_root),
